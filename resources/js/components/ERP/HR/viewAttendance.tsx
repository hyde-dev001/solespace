...
const ViewAttendance: React.FC = () => {
  ...
  const [selectedRecord, setSelectedRecord] = useState<(typeof mockAttendanceData)[0] | null>(null);
  ...
  const stats = useMemo(() => {
    ...
    const absent = mockAttendanceData.filter((a) => a.status === "absent").length;
    const leave = mockAttendanceData.filter((a) => a.status === "leave").length;
  ...
  const departments = Array.from(
    new Set(mockAttendanceData.map((a) => a.department))
  );
