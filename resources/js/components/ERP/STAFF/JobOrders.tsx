...
export default function JobOrdersPage() {
  ...
  const handleConfirmShipping = async () => {
    ...
    if (!etaPreset || !carrierCompany || !carrierName || !carrierPhone) {
      await Swal.fire({ title: "Missing details", text: "Please complete ETA and carrier details", icon: "warning", confirmButtonColor: "#2563eb" });
      return;
    }

    if (!trackingNumber || trackingNumber.trim() === "") {
      await Swal.fire({ title: "Tracking number required", text: "Please enter the tracking number provided by the courier.", icon: "warning", confirmButtonColor: "#2563eb" });
      return;
    }

    if (trackingLink && !isValidUrl(trackingLink)) {
      await Swal.fire({ title: "Invalid tracking URL", text: "Please enter a valid URL for the tracking link.", icon: "warning", confirmButtonColor: "#2563eb" });
      return;
    }
    ...
    try {
      ...
      // Update order locally after successful API call
      setOrders((prev) =>
        prev.map((o) =>
      ...
      await Swal.fire({ title: "Order Shipped", text: `Order ${selectedOrder.order_number} marked as shipped.`, icon: "success", confirmButtonColor: "#2563eb" });
      ...
      await Swal.fire({ 
        title: "Failed", 
        text: err.message || "Unable to mark as shipped. Try again.", 
        icon: "error", 
        confirmButtonColor: "#2563eb" 
      });
    }
  ...
  const handleCompleteOrder = async (order: Order) => {
    try {
      const result = await Swal.fire({
        ...
        html: `
          <div class="text-left">
            <p class="mb-2">Mark order <strong>${order.order_number}</strong> as completed?</p>
            <div class="bg-gray-50 dark:bg-gray-900 p-3 rounded-lg mb-4">
              <p class="text-sm"><strong>Customer:</strong> ${order.customer}</p>
              <p class="text-sm"><strong>Items:</strong> ${order.items.length} item(s)</p>
              <p class="text-sm"><strong>Amount:</strong> ${order.total}</p>
            </div>
            <p class="text-sm text-gray-600">After completion, you can generate an invoice for this order.</p>
          </div>
        `,
        icon: 'question',
        showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const completeResponse = await fetch(`/api/staff/orders/${order.id}/complete`, {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json',
          ...
          // Automatically create invoice after completing the order
          try {
            const response = await fetch('/api/finance/session/invoices/from-job', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
            ...
            await Swal.fire({
              title: 'Order Completed & Invoice Created!',
              html: `
                <div class="text-left">
                  <p class="mb-3">Order ${order.id} has been completed.</p>
                  <div class="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg mb-3">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                      ...
                      Amount: <strong>${invoice.total}</strong><br/>
                      Status: Draft
                    </p>
                  </div>
                  <p class="text-xs text-gray-500">
                    The Finance team can now review and finalize this invoice.
                  </p>
                </div>
              `,
              icon: 'success',
            ...
            // If invoice creation fails, still show order was completed
            await Swal.fire({
              title: 'Order Completed',
              html: `
                <div class="text-left">
                  <p class="mb-3">Order ${order.id} has been completed successfully.</p>
                  <div class="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg">
                    <p class="text-sm text-yellow-800 dark:text-yellow-200">
                      ⚠️ Invoice Creation Failed
                    </p>
                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-2">
                      ${error.message || 'Could not create invoice automatically.'}
                    </p>
                  </div>
                  <p class="text-xs text-gray-500 mt-3">
                    You can create the invoice manually from the Finance module.
                  </p>
                </div>
              `,
              icon: 'warning',
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {order.status === "shipped" && (
                            <button
                              ...
                              title="Complete order and create invoice"
                            >
                              Complete
                            </button>
                          )}
