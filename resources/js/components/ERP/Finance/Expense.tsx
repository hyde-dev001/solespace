...
const Expense: React.FC = () => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await api.patch(`/api/finance/expenses/${activeExpense.id}`, editForm);
  ...
  const confirmAndUpdateStatus = async (status: Expense["status"]) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const url = `/api/finance/expenses/${expenseId}/${endpoint}`;
  ...
  const handlePostToLedger = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const expenseId = activeExpense.id;
        const url = `/api/finance/expenses/${expenseId}/post`;
              ...
              {activeExpense.receipt_path && (
                <div className="flex flex-col gap-2 text-sm text-gray-700 dark:text-gray-300">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => window.open(`/api/finance/expenses/${activeExpense.id}/receipt/download`, '_blank')}
